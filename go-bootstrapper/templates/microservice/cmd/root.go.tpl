...
var (
	...
	root = &cobra.Command{
		Use:   "{{ .ProjectSlug }}",
		Short: "Microservice for {{ .ProjectName }}",
	}
)

func init() {
	cobra.OnInitialize(initConfig)
	root.PersistentFlags().StringVar(&configFile, "config", "", "config file (default is ${HOME}/.{{ .ProjectSlug }}.yaml")
}
...
func initConfig() {
	...
	if configFile != "" {
		...
		viper.SetConfigName(".{{ .ProjectSlug }}.yaml")
	}
